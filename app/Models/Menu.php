...
class Menu extends Model
{
	...
	public function __construct(array $attributes = [])
    {
        ...
        if(request()->segment(1) == 'admin' || request()->segment(1) == 'restaurant') {
	...
	public function getMenuTimeAttribute()
	{
		...
		else if ($restaurant_time = $this->RestaurantTime()) {
			return time_format($restaurant_time->start_time_for_english) . ' - ' . time_format($restaurant_time->end_time_for_english);
		}
	...
	public function getMenuStartTimeAttribute()
	{
		...
		if ($menu_time) {
			...
			return time_format($restaurant_time->start_time_for_english);
		}
	...
	public function getMenuEndTimeAttribute()
	{
		...
		if ($menu_time) {
			...
			return time_format($restaurant_time->end_time_for_english);
		}
	...
	public function getMenuClosedAttribute()
	{
		...
		if ($this->RestaurantTime()) {
			$restaurant_time = $this->RestaurantTime(); //restaurant opening time
			if ($restaurant_time->closed == 1) {
				return 1;
			}
			return 0;
		}
