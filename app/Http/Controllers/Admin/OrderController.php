...
class OrderController extends Controller {
	...
	public function view_order(Request $request)
	{
		...
		$rest = get_restaurant_user_id($this->view_data['order']->restaurant_id, 'user_id');
		$res_penality = Penality::where('user_id', $rest)->first();

		$this->view_data['restaurant_penality'] = 0;

		if ($res_penality) {
			$this->view_data['restaurant_penality'] = $res_penality->remaining_amount;
		}
	...
	public function cancel_push_notification($order_id) {
		...
		//restaurant
		push_notification($order->restaurant->user->device_type, $push_notification_title, $push_notification_data, $order->restaurant->user->type, $order->restaurant->user->device_id);
	...
	public function admin_payout() {
		...
		$rules = array(
			...
			'payout_to_restaurant' => 'numeric|nullable',
			'penalty_to_restaurant' => 'numeric|nullable',
		...
		$niceNames = array(
			...
			'payout_to_restaurant' => trans('admin_messages.payout_to_restaurant'),
			'penalty_to_restaurant' => trans('admin_messages.penalty_to_restaurant'),
		...
		if ($validator->fails()) {
			...
			// restaurant payout create or update
			if($request->payout_to_restaurant){
				$this->save_payout($order->id, $order->restaurant->user_id, $request->payout_to_restaurant, DEFAULT_CURRENCY);
			}
			if ($request->penalty_to_restaurant) {
				$this->save_penality('restaurant_penality', $order->id, $order->restaurant->user_id, $request->penalty_to_restaurant, DEFAULT_CURRENCY, isset($request->is_penalty_to_restaurant) ? $request->is_penalty_to_restaurant : '');
			}
...
// driver payout create or update
			...
			if ($request->payout_to_restaurant && $order->restaurant->user->payout_id) {
				$restaurant_status = $this->admin_payout_to_user($order->restaurant->user_id, $order->id);
				$status = $restaurant_status['success'];
				if ($status == false) {
					flash_message('danger', $restaurant_status['message']);
			...
			if($request->refund_to_eater > 0 ||$request->payout_to_restaurant > 0 ||$request->payout_to_driver > 0 )
				flash_message('success', $order->is_payout_create_or_not['message']);
			else if($request->penalty_to_eater > 0 || $request->penalty_to_restaurant > 0 ||$request->penalty_to_driver > 0 )
	...
	public function orders() {
		$this->view_data['form_name'] = trans('admin_messages.orders', ['restaurant_name' => '']);
	...
	public function sort_order() {
		...
		$total_earning = $complete_order->sum('booking_fee') + $complete_order->sum('restaurant_commision_fee') + $complete_order->sum('driver_commision_fee');
	...
	public function all_orders() {
		...
		$datatable = DataTables::of($orders)
			...
			->addColumn('restaurant_name', function ($orders) {
				return @$orders->restaurant->name;
			})
		...
		$columns = ['id', 'payment_type', 'user_name', 'restaurant_name', 'total', 'status_text'];
