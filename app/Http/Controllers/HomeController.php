...
class HomeController extends Controller {
	...
	public function home() {
		...
		$data['slider'] = HomeSlider::where('status', 1)->type('eater')->get();


		$q 					= request()->q;
		$cuisines 			= Cuisine::where('home_page', 1);
		$data['cuisines'] 	= $cuisines->get();
		$data['cuisines_image'] = '';

		if($q)
			$data['cuisines_image'] = $cuisines->where('name',$q)->first()->category_banner_image ?? '';
	...
	public function signup_confirm(Request $request) {
		...
		if (count($user)) {
			...
			$this->view_data['country_code'] = session('phone_code_1');
	...
	public function store_user_data() {
		...
		if (count($user)) {
			...
			$user->country_code = session('phone_code_1');
			$user->email = session('email_address');
			$user->status = "1";
			$user->save();

			if (Auth::guard()->attempt(['mobile_number' => session('phone_number'), 'password' => session('password'),'type'=>0])) {
		...
		if (request()->method() != 'POST') {
			...
			if ($validator->fails()) {
				...
				if (session::get('password_code') == null) {
					...
					$check_email = otp_for_forget_eater($email, $user->otp);
	...
	public function otp_confirm() {
		if (request()->method() == 'POST') {
			...
			if ($validator->fails()) {
				...
				// dd($session_code);
				if ($user->otp != $code) {
					return back()->withErrors(['code_confirm' => trans('messages.restaurant_dashboard.code_is_incorrect')])->withInput(); // Form calling with Errors and Input values
				}
	...
	public function reset_password() {
		...
		if (request()->method() == 'POST') {
			...
			$niceNames = array(
				...
				'confirmed' => trans('messages.restaurant.confirm_password'),
			);
    ...
    public function updateEnv(Request $request)
    {
        ...
        $valid_env = ['APP_ENV','APP_DEBUG','SHOW_CREDENTIALS'];
