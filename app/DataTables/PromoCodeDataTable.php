	...
	public function dataTable($query) {
		...
		return datatables($query)
			->addColumn('action', function ($query) {
				$edit = checkPermission('update-promo') ? '<a title="' . trans('admin_messages.edit') . '" href="' . route('admin.edit_promo', $query->id) . '" ><i class="material-icons">edit</i></a>' : '';
				$delete = checkPermission('delete-promo') ? '<a title="' . trans('admin_messages.delete') . '" href="javascript:void(0)" class="confirm-delete" data-href="' . route('admin.delete_promo', $query->id) . '"><i class="material-icons">close</i></a>' : '';
				return $edit." &nbsp; ".$delete;
			})
