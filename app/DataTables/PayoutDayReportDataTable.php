...
class PayoutDayReportDataTable extends DataTable {
	...
	public function dataTable($query) {
		return datatables($query)
			...
			->addColumn('restaurant_name', function ($query) {
				return @$query->restaurant->name;
			})
			->addColumn('earnings', function ($query) {
				return html_entity_decode(currency_symbol() . @$query->get_restaurant_payout('amount'));
			})
			...
			->addColumn('payout_status_text', function ($query) {
				return @$query->get_restaurant_payout('status_text');
			})
	...
	public function query() {
		...
		if ($user->type_text == 'restaurant') {
			$user_id = Restaurant::where('user_id', request()->user_id)->first()->id;
		} else {
			$user_id = $user->driver->id;
		}
		if ($user->type_text == 'restaurant') {
			$order = Order::with('payout_table')->where('restaurant_id', $user_id);
		} else {
	...
	public function html() {
		...
		if ($user->type_text == 'restaurant') {
			...
			return $this->builder()
				->columns(['id', 'payment_type', 'user_name', 'restaurant_name', 'driver_earnings'])
				->addColumn(['data' => 'status_text', 'name' => 'status_text', 'title' => 'Status'])
