	...
	public function dataTable($query) {
		...
		return datatables($query)
			->addColumn('action', function ($query) {
				$edit = checkPermission('update-static_page') ? '<a title="' . trans('admin_messages.edit') . '" href="' . route('admin.edit_static_page', $query->id) . '" ><i class="material-icons">edit</i></a>' : '';
				$delete = checkPermission('delete-static_page') ? '<a title="' . trans('admin_messages.delete') . '" href="javascript:void(0)" class="confirm-delete" data-href="' . route('admin.delete_static_page', $query->id) . '"><i class="material-icons">close</i></a>' : '';
				return $edit." &nbsp; ".$delete;
			})
