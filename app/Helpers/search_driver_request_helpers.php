...
if (!function_exists("searchDrivers")) {
	...
	function searchDrivers($order_id)
	{
		...
		list('latitude' => $pickup_latitude, 'longitude' => $pickup_longitude, 'address' => $pickup_location) = collect($order->restaurant->user_address)->only(['latitude', 'longitude', 'address'])->toArray();
...
if (!function_exists("searchRequestDriver")) {
	...
	{
		logger('request-helper '.$drop_location);
		clearPending(); 
		...
		if ($drivers->count() == 0) {
			...
			$restaurant = $order->restaurant->user;
		...
		if (!$request_already) {
			...
			$last_second = DriverRequest::where('driver_id', $nearest_driver->id)->where('status', '0')->count();
		...
		while ($active) {
			...
			if ($driver_accept != '') {
				...
				$user = $order->user;
				$push_notification_title = trans('api_messages.orders.your_food_preparation_done') . $order_id;
				$push_notification_data = [
