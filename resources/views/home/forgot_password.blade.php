	...
	<div class="container">
		...
		<div class="login-form py-5 mb-5 col-md-8 col-lg-6 mx-auto">
			...
			<form action="{{route('forgot_password')}}" method="POST">
				...
				<div class="mt-3">
					<p>{{trans('messages.profile.having_trouble')}}<span class="qust">{{ trans('messages.restaurant.ques_mark') }}</span> <a class="theme-color" href="{{route('help_page',current_page())}}">{{trans('messages.profile.get_help')}}</a></p>
				</div>
			</form>
		</div>
	</div>
