		...
		<div class="login-form py-5 mb-5 col-md-8 col-lg-6 mx-auto">
			...
			<form method="POST" action="{{ route('authenticate') }}">
				...
				<div class="form-group">
					...
					<input type="password" value="{{$password}}" name="password" id="password" placeholder=""/>
					<span class="text-danger"> {{$errors->first('password')}} </span>
				</div>
				<div class="forget_link">
					<a href="{{route('forgot_password')}}">{{trans('messages.profile.forget_password')}}<span class="qust">{{ trans('messages.restaurant.ques_mark') }}</span> </a>
