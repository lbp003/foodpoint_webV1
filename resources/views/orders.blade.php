			...
			<div class="tab-content" id="myTabContent">
				<div class="tab-pane fade show active" id="past" role="tabpanel" aria-labelledby="past-tab">
					<div class="order-list">
						...
						<div class="order-item text-center text-md-left d-md-flex">
							<div class="order-img" style="background-image: url('{{$order_row->restaurant->restaurant_image}}');">
							</div>
							<div class="order-info d-flex flex-column justify-content-between">
								<div class="shop-name">
									<h1><span>{{$order_row->restaurant->name}} </span>- <span> {{$order_row->restaurant->user_address->city}}</span></h1>
								...
								<div class="order-method">
									...
									<span>
										{{trans('admin_messages.delivery_mode')}} :  {{$order_row->delivery_mode_text}}
									</span>
									<span>
							...
							<div class="order-status d-flex flex-column text-md-right">
								...
								@endif
								@if($order_row->restaurant->status !=0)
								<a class="btn btn-theme mt-2" href="details/{{$order_row->restaurant->id}}">{{trans('messages.profile_orders.view_restaurant')}}</a>
								@endif
							</div>
						</div>
						@endforeach
						...
						<div class="order-item text-center text-md-left d-md-flex">
							...
							<div class="order-status d-flex flex-column justify-content-end text-md-right">
								<a href="stores" class="btn btn-theme mt-2 mt-md-0">{{trans('messages.profile_orders.view_restaurant')}}</a>
							</div>
						</div>
						@endif
					</div>
				...
				<div class="tab-pane fade" id="upcoming" role="tabpanel" aria-labelledby="upcoming-tab">
					...
					<div class="order-list">
						...
						<div class="order-item text-center text-md-left d-md-flex">
							<div class="order-img" style="background-image: url('{{$upcoming_order_row->restaurant->restaurant_image}}');">
							</div>
							<div class="order-info d-flex flex-column justify-content-between">
								<a href="{{route('order_track',['order_id'=>$upcoming_order_row->id])}}">
									<div class="shop-name">
										<h1><span>{{$upcoming_order_row->restaurant->name}} </span>-<span> {{$upcoming_order_row->restaurant->user_address->city}}</span></h1>
									...
									<div class="order-method">
										...
										<span>
										{{trans('admin_messages.delivery_mode')}} :  {{$upcoming_order_row->delivery_mode_text}}
										</span>
										<span>
							...
							<div class="order-status d-flex flex-column text-md-right">
								@if($upcoming_order_row->restaurant->status !=0)
								<a class="btn btn-theme mt-auto" href="details/{{$upcoming_order_row->restaurant->id}}">{{trans('messages.profile_orders.view_restaurant')}}</a>
								@endif
								...
								<h4>
									...
									<div class="modifier-info" ng-repeat="order_modifier in receipt_row.order_item_modifier">
										<div ng-repeat="order_modifier_item in order_modifier.order_item_modifier_item" >
											<span class="text-truncate"> @{{ order_modifier_item.name }} </span>
											<span class="d-inline-block text-nowrap ml-1"> ( <span ng-bind-html="currency_symbol"></span> @{{ order_modifier_item.price }} ) </span>
										</div>
									</div>
								</h4>
							</div>
						...
						<div class="checkout-total px-0">
							<div class="row d-block">
								<div class="d-flex align-items-center">
									<div class="col-7">
										<p>{{trans('messages.profile_orders.subtotal')}} (@{{order_detail.order_item.length}} {{ trans('messages.restaurant.item') }})</p>
									</div>
									<div class="col-5 text-right">
										<p><span ng-bind-html="currency_symbol"></span>@{{order_detail.subtotal}}</p>
									</div>
								</div>
								<div class="d-flex align-items-center" ng-show="order_detail.delivery_mode=='2'">
									<div class="col-7">
										<p>{{trans('messages.profile_orders.delivery_fee')}}</p>
									</div>
									<div class="col-5 text-right">
										<p ng-if="order_detail.delivery_fee>0">
											<span ng-bind-html="currency_symbol"></span>@{{order_detail.delivery_fee}}
										</p>
										<p ng-if="order_detail.delivery_fee==0">
