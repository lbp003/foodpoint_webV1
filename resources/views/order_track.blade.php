...
<main id="site-content" role="main" ng-controller="orders_detail">
	<div class="track-map mb-5">
		<div id="map" style="width:100%;height:400px;">
			@if($order_detail->delivery_mode != 1)
		...
		<div class="track-help">
			...
			<p>{{ trans('messages.restaurant.restaurant_name') }} : <a href="{{ url('details/'.$order_detail->restaurant_id)}} ">{{ $order_detail->restaurant->name }}</a></p>
		...
		<div class="container pt-5" id="order_loading">
			@if($order_detail->status!=4 && $order_detail->status!=2)
			<ul class="track-order clearfix mb-md-4 @if($order_detail->delivery_mode == 1) delivery_class @endif">
				<li class="{{($order_detail->status>=3)?'active':''}}">
					<div class="track-img">
						<img src="{{url('/')}}/images/1.png"/>
					</div>
					@if($order_detail->status==1)
					<p>{{ trans('messages.profile_orders.confirming_order_with_restaurant') }}
				...
				<li class="{{($order_detail->status>=6)?'active':''}}">
					...
					@if($order_detail->delivery_mode == 1)
 					<p>{{ trans('messages.profile_orders.food_is_ready_time_to_eat') }}</p>
 					@else					
					<p>{{ trans('messages.profile_orders.food_is_arriving_time_to_eat') }}</p>
					@endif
