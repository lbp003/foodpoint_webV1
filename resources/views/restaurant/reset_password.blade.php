	...
	<div class="container">
		...
		<div class="password-form py-5 mb-5 col-md-8 col-lg-6 mx-auto">
			...
			<form method="" action="">
				...
				<p id='password_count' style="display: none; color:red;">{{ trans('messages.restaurant_dashboard.password_must_be_atleast') }}</p>
				<p id='password_required' style="display: none; color:red;">{{ trans('messages.restaurant.this_field_is_required') }}</p><div class="form-group">
					<label>{{ trans('messages.restaurant.confirm_password') }}</label>
					<input type="password" name="confirm_password" id="confirm_password" placeholder="{{ trans('messages.driver.enter_password_again') }}"/>
				</div>

				<p id='password_error' style="display: none; color:red;">{{ trans('messages.restaurant_dashboard.password_doesnot_match') }}</p>
