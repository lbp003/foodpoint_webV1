...
@section('main')
<main id="site-content" role="main" ng-controller="restaurant_dashboard">
	<div class="partners">
		...
		<div class="verification-steps mt-3 mb-5 my-md-5">
			<div class="verify-head d-flex align-items-center">
				...
				<h2>{{trans('messages.restaurant_dashboard.verification_step')}}</h2>
			</div>
				...
				<ul>
					<li class="{{$document? 'completed':''}}">
						<a href="{{route('restaurant.profile','#document')}}">
							...
							{{trans('messages.restaurant_dashboard.add_document')}}
						</a>
					</li>
					<li class="{{$open_time? 'completed':''}}">
						<a href="{{route('restaurant.profile','#open_time')}}">
							<i class="icon icon-success"></i>
							<i class="icon icon-cancel-button"></i>
							{{trans('messages.restaurant_dashboard.add_open_time')}}
						</a>
					</li>
					<li class="{{$profile_step? 'completed':''}}">
						<a href="{{route('restaurant.profile')}}">
							<i class="icon icon-success"></i>
							<i class="icon icon-cancel-button"></i>
							{{trans('messages.restaurant_dashboard.complete_profile')}}
						</a>
					</li>
					<li class="{{$payout_preference? 'completed':''}}">
						<a href="{{route('restaurant.payout_preference')}}">
							<i class="icon icon-success"></i>
							<i class="icon icon-cancel-button"></i>
							{{trans('messages.restaurant_dashboard.add_payout_preference')}}
						</a>
					</li>
					...
					<li class="{{($menu && $payout_preference && $profile_step && $open_time && $document)? '':'d-none'}}">
						<a href="javascript:void(0)">
							<i class="icon icon-success"></i>
							<i class="icon icon-cancel-button"></i>
							{{trans('messages.restaurant_dashboard.waiting_for_approval')}}
						</a>
					</li>
				</ul>
			</div>
		...
		<div id="sales">
			<div class="d-md-flex align-items-center justify-content-between">
				<h1 class="title">{{trans('messages.restaurant_dashboard.sales')}}</h1>
				<ul class="nav nav-tabs" id="myTab" role="tablist">
					<li class="nav-item">
						<a class="nav-link active" id="weekly-tab" data-toggle="tab" href="#weekly" role="tab" aria-controls="weekly" aria-selected="true">7 {{trans('messages.restaurant_dashboard.days')}}</a>
					</li>
					<li class="nav-item">
						<a class="nav-link" id="monthly-tab" data-toggle="tab" href="#monthly" role="tab" aria-controls="monthly" aria-selected="false">30 {{trans('messages.restaurant_dashboard.days')}}</a>
					</li>
				</ul>
			</div>
			<div class="panel-content my-3 my-md-5">
				<div class="tab-pane fade active" id="weekly" role="tabpanel" aria-labelledby="weekly-tab">
					<div class="d-md-flex align-items-center justify-content-between">
						<div class="net-pay col-md-4">
							<h2> {{ $default_currency_symbol }} {{$last_seven_total_payouts}}</h2>
							<p>{{trans('messages.restaurant_dashboard.net_payout')}}</p>
						</div>
						...
						<div class="net-chart col-md-8 mt-5 mt-md-0">
							...
							@else
							<h3>{{trans('messages.restaurant_dashboard.last_seven_days')}}</h3>
							@endif
						</div>
					</div>
					@if(count($top_sale_thirty_days)>0)
					<div class="menu-items mt-5 dashboard_menu">
						<h3>{{trans('messages.restaurant_dashboard.selling_menu_items')}}</h3>
				...
				<div class="tab-pane fade active " id="monthly" role="tabpanel" aria-labelledby="monthly-tab">
					<div class="d-md-flex align-items-center justify-content-between">
						<div class="net-pay col-md-4">
							<h2>{{ $default_currency_symbol }}{{$last_thirty_total_payouts}}</h2>
							<p>{{trans('messages.restaurant_dashboard.net_payout')}}</p>
						</div>
						<div class="net-chart col-md-8 mt-5 mt-md-0">
							...
							@else
							<h3>{{trans('messages.restaurant_dashboard.last_thirty_days')}}</h3>
							@endif
				...
				<div id="service">
					<h1 class="title">{{trans('messages.restaurant_dashboard.service_quality')}}</h1>
					<div class="mt-3">
						<p class="light-color">{{trans('messages.restaurant_dashboard.speed_and_convenience',['site_name'=> site_setting('site_name')]) }}</p>
					</div>
					<div class="panel-content mt-3 my-md-5">
						<div class="text-right">
							<p class="light-color">{{trans('messages.restaurant_dashboard.based_on_past_30_days')}}</p>
						</div>
							...
							<div class="mt-4 d-block row d-lg-flex align-items-center">
								<div class="col-12 col-lg-6">
									<div class="accepted-hr d-md-flex align-items-center row">
										<div class="col-12 col-md-4">
											<p>{{trans('messages.restaurant_dashboard.accept_orders')}}</p>
										</div>
									...
									<div class="expected-hr d-md-flex align-items-center row mt-4 mt-md-0">
										<div class="col-12 col-md-4">
											<p>{{trans('messages.restaurant_dashboard.cancel_orders')}}</p>
										</div>
										<div class="col-12 col-md-7 offset-md-1 d-md-flex new_bar align-items-center">
								...
								<div class="col-12 col-lg-6 mt-4 mt-lg-0">
									...
									<div class="hrs-info pd-15">
										<h4>{{trans('messages.restaurant_dashboard.thanks_for_being_reliable')}}</h4>
										<p>{{trans('messages.restaurant_dashboard.you_fulfilling_all_orders')}}</p>
									</div>
									@endif
								</div>
							</div>
				...
				<div id="customer-satisfaction">
					<div class="d-md-flex justify-content-between align-items-center">
						<h1 class="title">{{trans('messages.restaurant_dashboard.customer_satisfaction')}}</h1>
						<p class="light-color">{{trans('messages.restaurant_dashboard.based_on_past_30_days')}}</p>
					</div>
					<div class="panel-content my-3 my-md-5">
						<div class="service-row">
							<div class="d-block row d-lg-flex align-items-center">
								<div class="col-12 col-lg-6">
									...
									<p>{{trans('messages.restaurant_dashboard.satisfaction_rating')}}</p>
								...
								<div class="col-12 col-lg-6 mt-4 mt-lg-0">
									<div class="hrs-info">
										<h5>{{trans('messages.restaurant_dashboard.see_what_people_are_saying')}}</h5>
										<p class="light-color">{{trans('messages.restaurant_dashboard.customers_like_your_food')}}</p>
									</div>
								</div>
							</div>
						</div>
				...
				<div class="ratings-table mb-4">
					<h5>{{trans('messages.restaurant_dashboard.ratings')}}</h5>
					<div class="table-responsive">
						<table>
							<thead>
								<tr>
									<th>{{trans('messages.restaurant_dashboard.item')}}</th>
									<th>{{trans('messages.restaurant_dashboard.satisfaction_rating')}}</th>
									<th>{{trans('messages.restaurant_dashboard.negative_feedback')}}</th>
									<th></th>
								</tr>
							</thead>
							<tbody>
							...
							@endif
								...
								<tr>
									<td colspan="4" class="text-center"> {{trans('messages.restaurant_dashboard.no_item_rating_found')}}</td>
								</tr>
								@endif
							</tbody>
						</table>
					...
					<div class="modal-body">
						<form class="form_valitate">
							...
							<div class="mt-3 pt-4 modal-footer px-0 text-right">
								<button data-dismiss="modal" type="cancel" class="btn btn-primary theme-color">{{trans('messages.restaurant_dashboard.close')}}</button>
							</div>
						</form>
					</div>
				</div>
