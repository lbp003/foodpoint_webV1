...
<main id="site-content" role="main" ng-controller="payout_preferences1" ng-cloak>
	<div class="partners">
		...
		<div class="partner-payments mt-4 mb-5">
			...
			<div class="my-4">
				...
				<div class="week-activity mt-3 d-md-flex">
					<div class="col-md-3">
						<span class="d-block light-color mb-2">{{trans('messages.restaurant_dashboard.net_earnings')}}</span>
					...
					<div class="col-md-3">
						<span class="d-block light-color mb-2">{{trans('messages.restaurant.next_payment')}}</span>
						<h4>{{date('d', strtotime('next monday')).' '.trans('messages.driver.'.date('M', strtotime('next monday')))}}</h4>
					</div>
					<!-- Owe Amount -->
					<div class="col-md-3">
						<span class="d-block light-color mb-2">{{trans('messages.new_changes.pending_owe_amount')}}</span>
						<h4>{!!default_currency_symbol()!!} {{@$current_week_owe_amount}}</h4>
					</div>
				</div>
			</div>
			<div class="payment-history my-5">
				...
				<div class="table-responsive">
					<table>
						<thead>
							<tr>
								<th>{{trans('messages.restaurant.week_of')}}</th>
								...
								<th>{{trans('messages.restaurant_dashboard.net_payout')}}</th>
								...
								<th>{{trans('messages.restaurant.weekly_statement')}}</th>
							</tr>
						</thead>
						<tbody>
					...
					<div class="p-4 text-center">
						<h4 class="m-0">{{trans('messages.restaurant.no_payouts_available')}} !</h4>
					</div>
					@endif
				</div>
				{{--
						...
						<nav aria-label="Page navigation example" class="my-3 ml-3">
							<ul class="pagination">
								<li class="page-item disabled">
									<a class="page-link" href="#" tabindex="-1">{{trans('messages.restaurant.previous')}}</a>
								</li>
								<li class="page-item">
				...
				<div class="bank-details my-5">
					<h5>
						{{trans('messages.restaurant.bank_account_details')}}  
					</h5>
					@if(!$payout_preference)
					<div class="col-12 mt-3 p-0">
						<a href="javascript:voi(0)" data-toggle="modal" data-target="#account_modal" class="btn btn-theme modal_popup">{{trans('messages.restaurant.add')}}</a>
					</div>
					@endif
					@if($payout_preference)
					<div class="table-responsive">
						<table>
							<thead>
								<tr>
									...
									<th>{{trans('messages.restaurant.details')}}</th>
									<th>{{trans('messages.profile_orders.status')}}</th>
							...
							<tbody>
								<tr class="main-list">
									...
									<td>{{trans('messages.restaurant.ready')}}</td>
			...
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header">
						{{trans('messages.restaurant.add_payout_method')}}
							...
							<div class="panel-body" ng-init="payout_country={{json_encode(old('payout_country') ?: '')}};payout_currency={{json_encode(old('currency') ?: '')}};country_currency={{json_encode($country_currency)}};mandatory={{ json_encode($mandatory)}};mandatory_field={{ json_encode($mandatory_field)}};old_currency='{{ old('currency') ? json_encode(old('currency')) : '' }}';payout_responce=''">
								...
								<div ng-if="mandatory_field[payout_country]['currency']" class="select-cls" id="currency_payout">
									<label for="payout_info_payout_currency">
										@lang('messages.restaurant.currency')
										<span class="required">*</span>
									</label>
									<div   class="select">
								...
								<div>
									<label class="" for="phone_number">
										{{trans('messages.restaurant.phone_number')}}
									...
									<div ng-if="payout_country == 'JP'" class="select-cls row-space-3">
										<label for="user_gender">
											{{trans('messages.restaurant.gender')}}
										</label>
										<div class="select">
									...
									<div ng-class="(payout_country == 'JP'? 'jp_form row':'')" class="clearfix row-space-2">
										<div class="country-info" ng-class="(payout_country == 'JP'? 'col-md-6 col-12':'')">
											<label ng-if="payout_country == 'JP'"><b>{{trans('messages.restaurant.address_kana')}}:</b></label>
											<div>
											...
											<div>
												<label ng-if="payout_country == 'JP'" for="payout_info_payout_address2">{{trans('messages.restaurant.town')}}<span style="color:red">*</span></label>
											...
											<div>
												<label for="payout_info_payout_state">{{trans('admin_messages.state')}} / {{trans('messages.restaurant.province')}}<span style="color:red">*</span></label>
												<span><i style="font-size:13px;">{{trans('messages.restaurant.state_note')}} (E.g. New York - NY)</i></span>
										...
										<div ng-if="payout_country == 'JP'" class="country-info col-md-6 col-12 mt-3 mt-md-0">
											<label><b>{{trans('messages.restaurant.address_kanji')}}:</b></label>
											<div>
												<label for="payout_info_payout_address2">{{trans('messages.profile.address')}} 1<span style="color:red">*</span></label>
											...
											<div>
												<label for="payout_info_payout_address2">{{trans('messages.restaurant.town')}}<span style="color:red">*</span></label>
									...
									<div ng-repeat="(field_name,validation) in mandatory_field[payout_country]">
										<div ng-if="field_name!='currency' && field_name!='iban'">
											<label ng-switch="mandatory_field[payout_country]['iban'] && field_name=='account_number'" class="" for="@{{field_name}}" >
												<span ng-switch-when='true'>  {{trans('messages.restaurant.iban_number')}}</span>
		...
		<div id="legal_document" class="legal_document">
				<label class="control-label required-label" >{{trans('admin_messages.document')}} ({{trans('messages.restaurant.jpg_or_png_format')}})<span style="color:red" id="document_smbl">*</span></label>
			...
			<span id="doc_valid" style="display: none">{{trans('js_messages.restaurant.field_required')}}</span>
		...
		<div class="legal_document">
			<label class="control-label required-label" >{{trans('admin_messages.additional_document')}} ({{trans('messages.restaurant.jpg_or_png_format')}})
			<span style="color:red">*</span></label>
			<div class="col-12 p-0">
			...
			<span id="add_doc_valid" style="display: none">{{trans('js_messages.restaurant.field_required')}}</span>
					...
					<div class="modal-cell">
						<div id="modal-add-payout-set-address" class="modal-content">
							...
							<div class="panel-header hide edit_payout_mtd">
								<a data-behavior="modal-close" class="panel-close" href="javascript:void(0);"></a>
								{{ trans('messages.restaurant.edit_payout_preference') }}
							</div>
							<div class="flash-container" id="popup1_flash-container"> </div>
						</div>
					</div>
						...
						<div id="modal-add-payout-set-address" class="modal-content">
							...
							<div class="flash-container hide" id="popup3_flash-container">
								<div class="alert alert-error alert-error alert-header">
									...
									<i class="icon alert-icon icon-alert-alt"></i>{{trans('messages.restaurant.valid_email')}}</div>
								</div>
								...
								<form method="post" id="payout_paypal" action="{{ url('users/payout_preferences/'.$user_id) }}" accept-charset="UTF-8">
									...
									<div class="panel-body">
										PayPal {{ trans('messages.restaurant.email_id') }}
		...
		var $payout_valitation = $('#payout_preference_submit').validate({
			...
			messages: {
				payout_country : { required : Lang.get('js_messages.restaurant.field_required')},
				currency : { required : Lang.get('js_messages.restaurant.field_required')},
				phone_number : { required : Lang.get('js_messages.restaurant.field_required')},
				gender : { required : Lang.get('js_messages.restaurant.field_required')},
				address1 : { required : Lang.get('js_messages.restaurant.field_required')},
				city : { required : Lang.get('js_messages.restaurant.field_required')},
				state : { required : Lang.get('js_messages.restaurant.field_required')},
				postal_code : { required : Lang.get('js_messages.restaurant.field_required')},
			},
			errorElement: "span",
