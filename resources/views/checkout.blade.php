...
@section('main')
<main id="site-content" role="main" ng-init="order_data = {{json_encode($order_detail_data)}};other_restaurant='no';schedule_time_value={{json_encode(time_data('schedule_time'))}};schedule_date_value='{{session('schedule_data')['date']?:date('Y-m-d')}}';schedule_time_set='{{session('schedule_data')['time']}}'; pickup_restaurant = '{{ $pickup_restaurant }}'; delivery_door = '{{ $delivery_door }}'; delivery_mode ='{{ session('schedule_data')['delivery_mode'] ?? 2 }}'" ng-controller="restaurants_detail" class="place_order_change">
			...
			<div class="clearfix">
				<div class="checkout-detail col-12 col-md-7 col-lg-8 pl-0 pr-0 pr-md-4 float-left">
					...
					<div class="panel-group">
						...
						<div class="card active user_check2">
							...
							<div class="card-body schedule3" ng-init="status='{{$schedule_data['status']}}'">
								<p>
									...
									<time>{{$restaurant_details->convert_mintime}}–{{$restaurant_details->convert_maxtime}} {{trans('messages.store.min')}}</time>
						...
						<div class="card active user_check2">
							<div class="card-header">
								<h3>3. {{ trans('admin_messages.delivery_mode') }}</h3>
							</div>
							<div class="card-body checkout_delievery deleivery_sect">
								<div class="form-check col-md-12 py-3 px-0" ng-show="pickup_restaurant">
									<label class="form-check-label">
										{{ Form::radio('delivery_mode', 1, '', ['id'=>'radio-1', 'ng-change'=>'order_store_changes(order_row.order_item_id)', 'ng-model'=>'delivery_mode']) }}
										<label for="radio-1" class="radio-label d-flex align-items-center">
											{{ trans('admin_messages.pickup_rest') }}
										</label>
									</label>
								</div>
								<div class="form-check col-md-12 py-3 px-0" ng-show="delivery_door">
									<label class="form-check-label">
										{{ Form::radio('delivery_mode', 2, '', ['id'=>'radio-2', 'ng-change'=>'order_store_changes(order_row.order_item_id)', 'ng-model'=>'delivery_mode']) }}
										<label  for="radio-2" class="radio-label d-flex align-items-center">
						...
						<div class="card active user_check2 location_section" ng-init="num_five= ( delivery_mode==2 ? 5 : 4 );" ng-show="delivery_mode==2">
							<div class="card-header">
								...
								<div class="check-location w-100 pl-0 pl-md-4 mt-4 mt-md-0">
									<div class="form-group">
										<div class="search-input">
											...
											<input type="hidden" id="restaurant_id" value="{{$restaurant_details->id}}">
						...
						<div class="card active user_check2 promo_loading">
							<div class="card-header">
								<h3>@{{num_five}} . @lang('messages.profile.payment') </h3>
							</div>
				...
				<div class="checkout mb-5 position-sticky col-12 col-md-5 col-lg-4 p-0 float-right" id="calculation_form" ng-cloak>
					<form>
						...
						<h2>{{$restaurant_details->name}} - {{$restaurant_details->user_address->city}}</h2>
						...
						<p id="payment-error" class="mt-2 payment-error text-danger text-center d-none"></p>
						@endif
					</form>
								...
								<h4 class="d-md-flex justify-content-between">
									<span class="col-md-7 p-0">
										...
										<div class="modifier-info d-flex" ng-repeat="modifier in order_row.modifier">
											...
											<span class="d-inline-block text-nowrap ml-1"> ( {!!$restaurant_details->currency->code!!} @{{modifier.price}} ) </span>
										</div>
									</span>
									<span class="col-md-5 d-inline-block text-md-right p-0">
										<span>{!!$restaurant_details->currency->code!!}</span>
										<span class="d-inline-block">
						...
						<div class="checkout-total">
							...
							<div id="subtotal">
								<div class="checkout-total d-flex align-items-center" ng-init="total_count_order = {{count(session('order_data') ?? [])-1}}">
									<div class="col-7">
										<h3>{{ trans('messages.profile_orders.subtotal') }} ( <span id="total_count_dat">@{{order_data.total_item_count }}</span> {{ trans('messages.restaurant.item') }})</h3>
									</div>
									<div class="col-5 text-right">
										<h3 ><span>{!!$restaurant_details->currency->code!!}</span> <span id="total_price_dat">@{{order_data.subtotal | number : 2 }}</span></h3>
									</div>
								</div>
							</div>
							<div class="d-flex align-items-center" >
								...
								<div class="col-5 text-right" >
									<p><span class="currency1">{!!$restaurant_details->currency->code!!}</span> <span id="tax_amoun">@{{order_data.tax | number : 2 }}</span></p>
								</div>
							</div>
							...
							<div class="d-flex align-items-center">
								...
								<div class="col-5 text-right">
									<p><span class="currency1">{!!$restaurant_details->currency->code!!}</span> <span id="booking_fee_amoun">@{{order_data.booking_fee | number : 2 }}</span></p>
								</div>
							</div>
							<div class="d-flex align-items-center" ng-if="delivery_mode!=1 && delivery_mode!=''">
								<div class="col-7">
									<p>{{ trans('messages.profile_orders.delivery_fee') }}</p>
								</div>
								<div class="col-5 text-right">
									<p ng-show="order_data.delivery_fee>0">
										<span class="currency1">{!!$restaurant_details->currency->code!!}</span> <span id="delivery_fee_amoun">@{{order_data.delivery_fee | number : 2 }}</span>
									</p>
									<p ng-show="order_data.delivery_fee==0">
										{{ trans('messages.new_changes.free') }}
									</p>	
								</div>
							</div>
							<div class="d-flex align-items-center" ng-show="order_data.penalty > 0">
								...
								<div class="col-5 text-right" >
									<p><span class="currency1">{!!$restaurant_details->currency->code!!}</span> @{{order_data.penalty}}</p>
								</div>
							</div>
							<div ng-show="order_data.promo_amount > 0" class="d-flex align-items-center" >
								...
								<div class="col-5 text-right" >
									<p>-<span class="currency1">{!!$restaurant_details->currency->code!!}</span> <span id="tax_amoun">@{{order_data.promo_amount | number : 2 }}</span></p>
								</div>
							</div>
							<div class="d-flex align-items-center">
								<div class="col-7">
								...
								<div class="col-5 text-right">
									<h3><span class="currency1">{!!$restaurant_details->currency->code!!}</span> <span id="total_price_dat">@{{order_data.total_price | number:'2'}}</span></h3>
								</div>
							</div>
