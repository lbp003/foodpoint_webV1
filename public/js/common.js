...
app.controller('restaurant_dashboard', ['$scope','$http','$timeout','$filter',function($scope,$http,$timeout,$filter) {
...
app.controller('restaurant_side_bar', ['$scope','$http','$timeout','$filter',function($scope,$http,$timeout,$filter) {
	...
	$(document).on('change', ".status_check", function(){
		...
		if(status==1) {
			$('.restaurant_status_avai').removeClass('d-none');
			$('.restaurant_status_unavai').addClass('d-none');
		} else {
			$('.restaurant_status_avai').addClass('d-none');
			$('.restaurant_status_unavai').removeClass('d-none');
		}
...
app.controller('header_controller', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
	...
	$('.schedule-option').click(function(){
		...
		$scope.schedule_status = Lang.get('js_messages.restaurant.'+status);
		$scope.$apply();
	...
	function initAutocompletes() {
		...
		autocompletes = new google.maps.places.Autocomplete(document.getElementById('location_search_new'),{ types: ['geocode'] });
	...
	function fetchMapAddress1(data) {
		...
		$http.post(url_search,{
			...
			if(urlParams.has('q'))
				prefix_url = '?q=' + urlParams.get('q');

			var url = getUrls('search') + prefix_url;
			window.location.href = url ;
		});
	}

	$('.delivery_mode_tab').click(function(){

		$('.schedule-dropdown').hide();
		$('#schedule-close-sm').click();

		var delivery_mode = $(this).attr('data-dmode');
		var statuss = (delivery_mode=='2') ? $scope.delivery_txt : $scope.pickup_txt;
		$scope.delivery_mode = delivery_mode;
		$scope.delivery_mode_status = statuss;
	    $scope.$apply();
		console.log( $scope.delivery_mode_status );
...
app.controller('home_page', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
	...
	function fetchMapAddress1(data) {
		...
		// Add Auto complete loading
		$('.autocomplete_form').addClass('loading');
	...
	var restaurant_valitAate = $("#eater_signup_form").validate({
		...
		messages: {
			...
			'password' : {  required : Lang.get('js_messages.restaurant.field_required') , minlength : Lang.get('js_messages.restaurant.please_enter_at_least_characters')},
			'email_address' : {  required : Lang.get('js_messages.restaurant.field_required') , email : Lang.get('js_messages.restaurant.valid_email_address')},
			'phone_number' : {  required : Lang.get('js_messages.restaurant.field_required') , minlength : Lang.get('js_messages.restaurant.mobile_number_must_have_digits') ,number:Lang.get('js_messages.restaurant.please_enter_valid_number')},
		},
		errorElement: "span",
	...
	$('#signup_form_submit').click(function(){
		...
		if (!$valid) {
			$restaurant_valitAate.focusInvalid();
			return false;
		}
...
app.filter('translations', ["$filter", function($filter) {
	return function(value) {
		return Lang.get('js_messages.restaurant.'+value);
	};
}])
...
app.controller('restaurants_search', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
	$scope.category_id ='';
	...
	function initAutocompletes() {
		autocompletes = new google.maps.places.Autocomplete(document.getElementById('location_search'),{ types: ['address','geocode'] });
		autocompletes.addListener('place_changed', fillInAddress1);
	}
	...
	$scope.search_result = function(){
		...
		$http.post(url,{
			...
			$scope.restaurant_data = response.data;
...
$('#top_category_search, #top_category_search_mob').each(function() { 
	...
	$('.search-field .icon-close-2').click(function() {
		$this.attr("placeholder", Lang.get('js_messages.restaurant.search_for_restaurant_cuisine'));
	});   
});
